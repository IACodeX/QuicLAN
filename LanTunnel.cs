...
internal static partial class Program
{
    private sealed unsafe class WintunAdapter : IDisposable
    {
        ...
        public WintunSession StartSession(uint capacity = 0x400000)
        {
            ...
            if (session == IntPtr.Zero) throw new Win32Exception(Marshal.GetLastWin32Error(), "No se pudo iniciar sesion Wintun.");
    ...
    private sealed unsafe class WintunSession : IDisposable
    {
        ...
        public void SendPacket(byte[] packet)
        {
            ...
            if (dst == null) throw new Win32Exception(Marshal.GetLastWin32Error(), "WintunAllocateSendPacket fallo.");
    ...
    private static uint ParsePrefix16(string text)
    {
        ...
        if (slash >= 0)
        {
            ...
            if (!int.TryParse(bitsText, out int bits) || bits != DefaultPrefixBits)
                throw new ArgumentException($"--prefix solo admite redes /{DefaultPrefixBits}.");
        }
        if (!IPAddress.TryParse(ipText, out var ip) || ip.AddressFamily != AddressFamily.InterNetwork)
            throw new ArgumentException("--prefix debe ser una IPv4 valida, por ejemplo 10.88.0.0 o 10.88.0.0/16.");
    ...
    private static byte[] MakeTrackerPeerId(PeerId localId)
    {
        ...
        Encoding.ASCII.GetBytes("-QL0554-").CopyTo(peerId, 0);
    ...
    private static void ValidateAdapterName(string name)
    {
        if (string.IsNullOrWhiteSpace(name) || name.Length > 64 || name.Any(c => char.IsControl(c) || c is '"' or '\r' or '\n'))
            throw new ArgumentException("Nombre de adaptador invalido.", nameof(name));
    }
    ...
    private static void RunNetsh(string arguments)
    {
        ...
        if (!p.WaitForExit(10_000))
        {
            ...
            throw new TimeoutException("netsh tardo demasiado en responder.");
        }
        if (p.ExitCode != 0)
        {
            string err = p.StandardError.ReadToEnd();
            string output = p.StandardOutput.ReadToEnd();
            throw new InvalidOperationException($"netsh fallo: {err}{output}");
        }
    }
}
