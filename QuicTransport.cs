    ...
    private sealed partial class QuicLanNode
    {
        ...
        private async Task RunQuicListenerAsync(CancellationToken ct)
        {
            if (!QuicListener.IsSupported || !QuicConnection.IsSupported)
            {
                ...
                ConsolePanelHub.PublishLog("QUIC no soportado por este runtime/sistema; usando UDP fallback si esta habilitado.");
                return;
            }
            ...
            ConsolePanelHub.PublishLog($"QUIC listener activo en 0.0.0.0:{_options.DataPort}");
        ...
        private async Task EnsureQuicConnectionAsync(PeerState peer, CancellationToken ct)
        {
            ...
            try
            {
                ...
                connection = await QuicConnection.ConnectAsync(BuildClientOptions(peer), timeout.Token);
                stream = await connection.OpenOutboundStreamAsync(QuicStreamType.Bidirectional, timeout.Token);
                ...
                ConsolePanelHub.PublishLog($"QUIC conectado con {peer.Name} en {peer.QuicEndPoint}");
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { }
            catch (Exception ex)
            {
                LogVerbose($"QUIC connect {peer.Name}: {ex.Message}");
                ConsolePanelHub.PublishLog($"QUIC fallo con {peer.Name}: {ex.Message}. Datos usaran UDP fallback si esta habilitado.");
            }
            finally
            {
                if (!registered)
                ...
                _quicConnecting.TryRemove(peer.Id, out _);
            }
        }
        ...
        private async Task RunQuicAcceptedConnectionAsync(QuicConnection connection, CancellationToken outerCt)
        {
            try
            {
                ...
                if (remoteIdentity is null)
                    throw new AuthenticationException("Preface QUIC invalido.");
                ...
                if (!VerifyQuicIdentityFrame(remoteIdentity.Value, peer))
                    throw new AuthenticationException("Firma QUIC invalida.");
                RegisterQuicLink(peer, connection, stream, _shutdown.Token);
                LogVerbose($"QUIC entrante de {peer.Name}");
                ConsolePanelHub.PublishLog($"QUIC entrante conectado de {peer.Name}");
            }
            catch (OperationCanceledException) when (outerCt.IsCancellationRequested) { }
            catch (Exception ex)
            {
                LogVerbose($"QUIC incoming: {ex.Message}");
                ConsolePanelHub.PublishLog($"QUIC entrante fallido: {ex.Message}");
