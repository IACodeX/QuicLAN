...
internal static partial class Program
{
    ...
    private const string AppVersion = "5.5.4";
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        if (IsPanelProcess(args))
            return await RunPanelProcessAsync(args);
        ...
        {
            Console.Error.WriteLine("QuicLAN usa Wintun. Esta version es solo para Windows.");
            PauseIfInteractive(args);
            return 1;
        }
        ...
        try
        {
            ...
            await using var panels = await ConsolePanelManager.StartAsync(node, options, appCts.Token);
    ...
    private static void PrintControlHeader(Options options, string localIpText, PeerId localId)
    {
        Console.Clear();
        Console.WriteLine($"QuicLAN v{AppVersion} - CONTROL");
        Console.WriteLine("============================================================");
        Console.WriteLine($"Nombre visible : {options.Name}");
        Console.WriteLine($"Sala           : sha256:{Fingerprint(Encoding.UTF8.GetBytes(options.Room))}");
        Console.WriteLine($"Tu identidad   : {localId}");
        Console.WriteLine($"Tu IP virtual  : {localIpText}/{DefaultPrefixBits}  (reservada automatica)");
        Console.WriteLine();
        Console.WriteLine("Estado inicial : LOBBY");
        Console.WriteLine("LAN            : no activa");
        Console.WriteLine("Modo           : esperando propuesta");
        Console.WriteLine();
        Console.WriteLine("Ajustes actuales");
        Console.WriteLine($"  UDP discovery : 0.0.0.0:{options.Port}");
        Console.WriteLine($"  QUIC datos    : 0.0.0.0:{options.DataPort}" + (options.DisableQuic ? " (apagado)" : ""));
        Console.WriteLine($"  MTU / pak / bst: {options.Mtu} / {options.Pak} / {options.Burst}");
        Console.WriteLine($"  Red virtual   : {IpToString(options.Prefix)}/{DefaultPrefixBits}");
        string dataTransport = options.DisableQuic
            ? "UDP cifrado"
            : options.LegacyUdpData ? "QUIC + UDP fallback" : "QUIC sin UDP fallback";
        Console.WriteLine($"  Transporte    : {dataTransport}");
        Console.WriteLine();
        Console.WriteLine("Ventanas abiertas");
        Console.WriteLine("  Estado/Lobby  -  Logs/Diagnostico  -  Chat");
        Console.WriteLine();
        Console.WriteLine("Flujo simple");
        Console.WriteLine("  create server     propone una LAN tipo servidor/host");
        Console.WriteLine("  create red        propone una LAN de todos con todos");
        Console.WriteLine("  s / ready         marcarte listo para la propuesta o unirte a LAN activa");
        Console.WriteLine("  start             iniciar la LAN propuesta si tu la creaste");
        Console.WriteLine("  n / lobby         salir de LAN o quitar listo, sin salir de la sala");
        Console.WriteLine("  stat              ver estado y configuracion actual");
        Console.WriteLine("============================================================");
        Console.WriteLine("Estas en lobby. Espera una propuesta o escribe create server / create red.");
    }
    ...
    private sealed partial class QuicLanNode : IAsyncDisposable
    {
        ...
        private int _localLanReady;
        private readonly object _lobbyLock = new();
        private LobbyProposal? _activeProposal;
        private LobbyProposal? _ownedProposal;
        private LanSessionInfo? _activeLan;
        private LobbyMemberState _localLobbyState = LobbyMemberState.Lobby;
        private string _localIpMode = "auto";
        ...
        public static async Task<QuicLanNode> StartAsync(Options options, CancellationToken ct)
        {
            ...
            PrintControlHeader(options, localIpText, localId);
            ConsolePanelHub.PublishLog($"nodo iniciado: nombre={options.Name}, ip={localIpText}, id={localId}");
        ...
        private bool IsLocalLanReady => Volatile.Read(ref _localLanReady) == 1;
        ...
        private Task RunGuardedAsync(string name, Func<CancellationToken, Task> body) => Task.Run(async () =>
        {
            ...
            {
                _fatal = ex;
                Console.Error.WriteLine($"Tarea '{name}' fallo: {ex.Message}");
                _shutdown.Cancel();
            }
        });
                ...
                while (!ct.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        if (IsOverlayBroadcast(destination) || IsMulticast(destination))
                            targets = _peers.Values.Where(ShouldSendLanPacketToPeer).ToArray();
                        else if (_routes.TryGetValue(destination, out var peer) && ShouldSendLanPacketToPeer(peer))
                            targets = [peer];
                    ...
                    {
                        bool preferQuic = IsOverlayBroadcast(destination) || IsMulticast(destination) || targets.Length == 1;
                        foreach (var target in targets)
                            QueueOrSendIpPacket(target, copy, preferQuic);
                    }
                }
            }
        }
        ...
        private void QueueOrSendIpPacket(PeerState target, byte[] payload, bool preferQuic)
        {
            ...
            if (!_options.DisableQuic && preferQuic && target.QuicLink is { IsOpen: true } quic && quic.TryQueue(payload))
            {
            ...
            if (!_sendQueue.Writer.TryWrite(new SendJob(target, payload, preferQuic)))
                Interlocked.Increment(ref _droppedPackets);
        }
        ...
        private async Task SendIpJobAsync(SendJob job, CancellationToken ct)
        {
            ...
            if (!ShouldSendLanPacketToPeer(peer) || crypto is null) return;
            ...
            if (!_options.DisableQuic && job.PreferQuic && peer.QuicLink is { IsOpen: true } quic && quic.TryQueue(job.Payload))
            {
                Interlocked.Increment(ref _txPackets);
                Interlocked.Increment(ref _quicPackets);
                return;
            ...
            while (await timer.WaitForNextTickAsync(ct))
            {
                ...
                foreach (var peer in _peers.Values)
                {
                    ...
                    if (idleMs > PeerTimeout.TotalMilliseconds && peer.MarkDisconnected())
                    {
                        ...
                        ConsolePanelHub.PublishLog($"peer reconectando/fantasma: {peer.Name} {IpToString(peer.VirtualIp)}. Su IP queda reservada temporalmente.");
                    }
                ...
                LanSessionInfo? activeSession;
                lock (_lobbyLock) activeSession = _activeLan;
                if (activeSession is not null && activeSession.Mode == LanMode.Server && !activeSession.HostId.Equals(_localId))
                {
                    if (!_peers.TryGetValue(activeSession.HostId, out var hostPeer) || !hostPeer.IsOnline)
                    {
                        lock (_lobbyLock)
                        {
                            if (_activeLan?.SessionId == activeSession.SessionId)
                            {
                                _activeLan = null;
                                SetLocalMemberStateLocked(LobbyMemberState.Lobby);
                            }
                        }
                        ConsolePanelHub.PublishLog($"host perdido: {activeSession.HostName}. La sala sigue viva en lobby.");
                        _ = BroadcastLocalLobbyStatusAsync(_shutdown.Token);
                    }
                }

                await BroadcastCurrentLobbySnapshotAsync(ct);
        ...
        private void ApplyHello(PeerState peer, HelloInfo hello, IPEndPoint remote)
        {
            ...
            {
                LogVerbose($"Hello desde endpoint no fisico ignorado para transporte: {remote}");
            }
            ...
            {
                if (hello.VirtualIp == GetLocalIp() && !_localIpMode.Equals("custom", StringComparison.OrdinalIgnoreCase) && peer.Id.CompareTo(_localId) < 0)
            ...
            if (becameOnline)
                ConsolePanelHub.PublishLog($"peer en lobby: {peer.Name} {IpToString(peer.VirtualIp)} via {remote}");
            else if (ipChanged)
                ConsolePanelHub.PublishLog($"peer actualizo IP virtual: {peer.Name} {IpToString(previousIp)} -> {IpToString(peer.VirtualIp)}");
        ...
        {
            if (!ShouldSendLanPacketToPeer(peer))
            {
                Interlocked.Increment(ref _droppedPackets);
                return;
            }
    ...
    private sealed record class Options
    {
        ...
        public static Options Parse(string[] args)
        {
            ...
            if (string.IsNullOrWhiteSpace(o.Room)) throw new ArgumentException("Falta --room. Dos o mas PCs deben usar la misma sala/clave.");
            ...
            if (string.IsNullOrWhiteSpace(o.Name)) throw new ArgumentException("Falta --name. Es obligatorio elegir un nombre visible antes de entrar a la sala.");
            if (o.Room.Length < 8) Console.Error.WriteLine("Aviso: una sala corta es facil de adivinar. Usa una frase larga.");
            if (!dataPortExplicit)
            {
                int inferredDataPort = o.Port == 65535 ? 65534 : o.Port + 1;
                o = o with { DataPort = inferredDataPort };
            }
            if (!o.DisableQuic && o.DataPort == o.Port) throw new ArgumentException("--data-port debe ser distinto de --port cuando QUIC esta activo.");
            if (o.DisableQuic && !o.LegacyUdpData) throw new ArgumentException("--disable-quic y --no-udp-data-fallback no pueden usarse juntos: dejaria el trafico IP sin transporte de datos.");
        ...
        {
            Console.Clear();
            Console.WriteLine($"QuicLAN v{AppVersion}");
            Console.WriteLine("Modo simple: escribe nombre + sala y listo. Todo lo demas va en automatico.\n");

            string name;
            do
            {
                Console.Write("Nombre visible: ");
            ...
            var args = new List<string> { "--name", name, "--room", room };
            Console.WriteLine();
            Console.WriteLine("Entrando al lobby... La LAN no se activa hasta que exista una propuesta y alguien escriba start.");
...
{AppName} v{AppVersion} - LAN virtual sobre QUIC real + discovery UDP + Wintun
...
Opciones:
  ...
  --name <nombre>      Nombre visible obligatorio. Nunca se usa el nombre del PC por defecto.
  ...
  --pak <bytes>        Limite de paquete overlay/IP. Por defecto: {DefaultPak}. Rango: {MinPak}-{MaxPak}.
  --bst <n>            Rafaga de envio interna. Por defecto: {DefaultBurst}. Rango: {MinBurst}-{MaxBurst}.
  --prefix <ipv4>      Red virtual /16. Por defecto: {IpToString(DefaultPrefix)}.
  --no-trackers        Solo descubrimiento por LAN local; no usa trackers publicos.
  --tracker <url>      Anade tracker UDP estilo udp://host:port/announce.
  --verbose            Muestra diagnostico resumido.
  --no-elevate         No relanza como administrador.
  --disable-quic       Desactiva QUIC y usa solo el transporte legacy UDP cifrado.
  --no-udp-data-fallback No usa fallback UDP para datos si QUIC aun no esta listo.

Durante la ejecucion puedes escribir en la terminal principal: stat, peers, pak 1000, bst 16, mtu 1200, reset, help o quit.

Ejemplo:
  QuicLan.exe --name "Pedro Minecraft" --room "familia-2026-frase-larga"
